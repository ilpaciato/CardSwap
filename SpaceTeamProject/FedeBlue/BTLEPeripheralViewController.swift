...
class BTLEPeripheralViewController: UIViewController, CBPeripheralManagerDelegate, UITextViewDelegate {
    ...
    @IBOutlet weak var claudio_switch: UISwitch!
...
//        let jsonEncoder = JSONEncoder()
        let jsonData = try! jsonEncoder.encode(claudio)
    ...
    func peripheralManager(_ peripheral: CBPeripheralManager, central: CBCentral, didSubscribeTo characteristic: CBCharacteristic) {
        ...
        dataToSend = jsonString.data(using: String.Encoding.utf8)
        // Reset the index
        sendDataIndex = 0;
    ...
    @IBAction func Claudioswitchchanged(_ sender: UISwitch) {
       advertisingSwitch.isOn = false
        if claudio_switch.isOn {
          let jsonData = try! jsonEncoder.encode(claudio)
            jsonString = String(data: jsonData, encoding: .utf8)!
            print(jsonString)
            // All we advertise is our service's UUID
